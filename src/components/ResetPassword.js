    ...
    if (!password.trim() || !confirmPassword.trim()) {
      switch (false) {
        ...
        case Boolean(confirmPassword.trim()):
          return setMessage({ ...message, warning: "Confirm password field is empty" })
        default:
      }
    }
    try {
  ...
  return (
    <div className="recipeform resetPassword" >
      <form onSubmit={handleSubmit}>
        ...
        <div className="field">
          <label className={`${mode}`} htmlFor="password">New Password:</label>
          <input required autoComplete="off" name="password" onChange={handleChange("password")} value={password} type="password" id="password1" />
        </div>
        <div className="field">
        ...
        {
          !isLoading ?
            <button style={{ backgroundColor: `${navColor}`, color: "#fff" }} className="submit" >Reset password</button>
            : <button disabled className="signin loading" style={{ margin: "auto", width: "150px", height: "32px", backgroundColor: `${navColor}40`, color: "#fff" }} ><CircularProgress style={{ marginTop: "3px", width: "15px", height: "15px" }} /></button>
        }
      </form >
    </div >
  )
